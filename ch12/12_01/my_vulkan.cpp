...
VkResult vk_init_instance()
{
    ...
    appInfo.pApplicationName = "Chapter 12.1 - Queries";
...
{
    vk_query_pool pool = {};
    VkResult vkr = vk_create_query_pool(queryType, queryCount, &pool);
    switch (vkr)
    {
    case VK_SUCCESS:
        std::cout << "Query pool created successfully.\n";
        vkr = vk_track_query_pool(&pool);
        if (vkr == VK_SUCCESS)
            std::cout << "Query pool tracked successfully.\n";
        else
            std::cout << "Warning: Query pool tracking failed.\n";
        break;
    case VK_ERROR_INITIALIZATION_FAILED:
        std::cout << "Warning: Query pool creation failed.\n";
        break;
    default:
        std::cout << "Warning! vk_create_query_pool error=" << vkr << "\n";
    }
}
...
void my_reset_and_query(void)
{
    ...
    // Record command buffer with query operations
    ...
    // Reset query pool
    vkCmdResetQueryPool(m_commandBuffer, pool->handle, 0, pool->queryCount);

    // Begin query (simulating the draw operation from the book)
    vkCmdBeginQuery(m_commandBuffer, pool->handle, 0, 0);

    // In a real application, a render pass and draw call would happen here.
    // For this demonstration, we skip the render pass and just end the query.

    vkCmdEndQuery(m_commandBuffer, pool->handle, 0);
    ...
    // Read query results
    uint64_t samples = 0;
    vkr = vkGetQueryPoolResults(m_device, pool->handle, 0, 1, sizeof(samples), &samples,
                                 sizeof(samples), VK_QUERY_RESULT_64_BIT);
    if (vkr != VK_SUCCESS)
    {
    ...
    {
        std::cout << "Query result: " << samples << " samples\n";
    }
