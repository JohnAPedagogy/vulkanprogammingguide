...
VkResult vk_init_instance(void)
{
    ...
    appInfo.pApplicationName = "Ch04_01 Image Barriers";
                           ...
                           VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask)
{
    ...
    vkCmdPipelineBarrier(cmdBuf, srcStageMask, dstStageMask, 0,
...
void my_init_vulkan(void)
{
    ...
    {
    case VK_SUCCESS:
        std::cout << "Queue family " << m_queueFamilyIndex << " selected.\n";
        break;
    case VK_ERROR_INITIALIZATION_FAILED:
...
{
    // Create a 2D image for transfer operations
    vk_image img;
    ...
    if (vkr != VK_SUCCESS)
    {
        std::cout << "Warning: Image creation failed.\n";
        switch (vkr)
        {
        case VK_ERROR_INITIALIZATION_FAILED:
    ...
    if (vkr != VK_SUCCESS)
    {
        std::cout << "Warning: Image memory tracking failed.\n";
        switch (vkr)
        {
        case VK_ERROR_INITIALIZATION_FAILED:
    ...
    std::cout << "Image created: " << img.extent.width << "x" << img.extent.height << "\n";

    // Create a command buffer and record barrier
    ...
    if (vkr != VK_SUCCESS)
    {
        ...
        case VK_ERROR_INITIALIZATION_FAILED:
            ...
            break;
        default:
            std::cout << "vk_create_command_buffer error=" << vkr << "\n";
        }
        return;
    }
    ...
    {
        // Record a barrier transitioning from TRANSFER_DST to SHADER_READ
        vkr = vk_record_barrier(cmdBuf.handle, img.handle,
        ...
        if (vkr == VK_SUCCESS)
        {
            std::cout << "Barrier recorded: TRANSFER_DST -> SHADER_READ_ONLY\n";
        }
        else
        {
            std::cout << "Warning: vk_record_barrier failed with result=" << vkr << "\n";
        }
...
{
    std::cout << "Image info: " << img->extent.width << "x" << img->extent.height
              << "x" << img->extent.depth << "\n";
}
