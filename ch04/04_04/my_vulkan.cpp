...
VkResult vk_init_instance(void)
{
    ...
    appInfo.pApplicationName = "Ch04_04 Blit & Resolve";
...
// ---- vk_* image operations - resolve and blit ----
                                 ...
                                 VkImageLayout dstLayout, uint32_t width, uint32_t height)
{
    ...
    vkCmdResolveImage(cmdBuf, srcImage, srcLayout, dstImage, dstLayout, 1, &region);
                              ...
                              uint32_t dstWidth, uint32_t dstHeight)
{
    ...
    region.dstOffsets[1] = {(int32_t)dstWidth, (int32_t)dstHeight, 1};

    vkCmdBlitImage(cmdBuf, srcImage, srcLayout, dstImage, dstLayout, 1, &region, VK_FILTER_LINEAR);
...
{
    // Create multisampled source image
    vk_image msaaImg;
    ...
    if (vkr != VK_SUCCESS)
    {
        std::cout << "Warning: MSAA image creation failed.\n";
        switch (vkr)
        {
        case VK_ERROR_INITIALIZATION_FAILED:
    ...
    if (vkr != VK_SUCCESS)
    {
        std::cout << "Warning: MSAA image memory tracking failed.\n";
        switch (vkr)
        {
        case VK_ERROR_INITIALIZATION_FAILED:
    ...
    std::cout << "MSAA image created: " << msaaImg.extent.width << "x" << msaaImg.extent.height
              << ", samples=" << msaaImg.samples << "\n";

    // Create single-sample resolved image
    vk_image resolvedImg;
    ...
    if (vkr != VK_SUCCESS)
    {
        std::cout << "Warning: Resolved image creation failed.\n";
        return;
    }

    vkr = vk_track_image(&resolvedImg, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    if (vkr != VK_SUCCESS)
    {
        std::cout << "Warning: Resolved image memory tracking failed.\n";
        return;
    }

    std::cout << "Resolved image created: " << resolvedImg.extent.width << "x" << resolvedImg.extent.height << "\n";

    // Create mip image for blit destination
    vk_image mipImg;
    vkr = vk_create_image(128, 128, VK_FORMAT_R8G8B8A8_SRGB,
                          VK_IMAGE_USAGE_TRANSFER_DST_BIT,
                          VK_SAMPLE_COUNT_1_BIT,
                          &mipImg);
    if (vkr != VK_SUCCESS)
    {
        std::cout << "Warning: Mip image creation failed.\n";
        return;
    }

    vkr = vk_track_image(&mipImg, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    if (vkr != VK_SUCCESS)
    {
        std::cout << "Warning: Mip image memory tracking failed.\n";
        return;
    }

    std::cout << "Mip image created: " << mipImg.extent.width << "x" << mipImg.extent.height << "\n";
    ...
    // Record resolve operation
    vkr = vk_record_resolve_image(cmdBuf.handle, msaaImg.handle,
    ...
    if (vkr == VK_SUCCESS)
    {
        std::cout << "Resolve command recorded: 4x MSAA -> 1x\n";
    }
    else
    {
        std::cout << "Warning: vk_record_resolve_image failed with result=" << vkr << "\n";
    }

    // Record blit operation
    vkr = vk_record_blit_image(cmdBuf.handle, resolvedImg.handle,
                               VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                               256, 256,
                               mipImg.handle,
                               VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    if (vkr == VK_SUCCESS)
    {
        std::cout << "Blit command recorded: 256x256 -> 128x128 (filtered)\n";
    }
    else
    {
        std::cout << "Warning: vk_record_blit_image failed with result=" << vkr << "\n";
    }
