...
# Shader compilation: glslc compiles GLSL shaders to SPIR-V (.spv files)
# placed next to the built executable for runtime loading.
...
SHADER_SOURCES += shaders/tessellation.vert shaders/tessellation.tesc shaders/tessellation.tese
...
# the run PATH. Also copy compiled SPIR-V shaders.
dlls_copy.commands = $(COPY) \"$$shell_path($$MSYS2_DIR/bin/glfw3.dll)\" \"$$shell_path($$OUT_PWD/$$DEST_SUBDIR)\" $$escape_expand(\\n\\t) \
                     ...
                     $(COPY_DIR) \"$$shell_path($$PWD/shaders)\" \"$$shell_path($$OUT_PWD/$$DEST_SUBDIR/shaders)\"
