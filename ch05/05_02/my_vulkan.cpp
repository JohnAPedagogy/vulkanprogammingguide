...
VkResult vk_create_swapchain(uint32_t width, uint32_t height, VkSwapchainKHR *outSwapchain)
{
    ...
    // Guard clauses for required initialization
    ...
    // Select a format (use first available)
    ...
    // Use FIFO present mode (always available, vsync)
    VkPresentModeKHR presentMode = VK_PRESENT_MODE_FIFO_KHR;
    free(presentModes);

    // Determine image count (min + 1, but respect capabilities)
...
{
    VkResult vkr = vk_create_window(800, 600, "Vulkan - Swap Chain", &m_window);
