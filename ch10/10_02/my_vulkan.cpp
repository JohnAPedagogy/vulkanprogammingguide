...
VkResult vk_device_init_count(int *count)
{
    *count = 0;
    ...
    appInfo.pApplicationName = "Ch10 Fragment Tests";
...
VkResult vk_create_image_view(vk_image *image)
{
    ...
    VkImageAspectFlags aspect = (image->format == VK_FORMAT_D32_SFLOAT_S8_UINT ||
...
VkResult vk_create_render_pass_with_depth(VkRenderPass *outRenderPass)
{
    VkResult vkr = VK_INCOMPLETE;
...
VkResult vk_create_framebuffer_with_depth(VkRenderPass renderPass, uint32_t width, uint32_t height,
...
VkResult vk_cleanup()
{
    ...
    {
        if (rt.depthStencil.view != VK_NULL_HANDLE)
            vkDestroyImageView(m_device, rt.depthStencil.view, &g_renderAllocCallbacks);
        if (rt.depthStencil.handle != VK_NULL_HANDLE)
        {
            vkDestroyImage(m_device, rt.depthStencil.handle, &g_renderAllocCallbacks);
            if (rt.depthStencil.memory != VK_NULL_HANDLE)
                vkFreeMemory(m_device, rt.depthStencil.memory, &g_renderAllocCallbacks);
        }
        if (rt.colorView != VK_NULL_HANDLE)
            vkDestroyImageView(m_device, rt.colorView, &g_renderAllocCallbacks);
...
void my_init_vulkan()
{
    ...
    if(rc == VK_SUCCESS) {
        ...
        std::cout << "Device initialization failed with error " << rc << "\n";
    }
}
...
void my_create_depth_test_target(void)
{
    ...
    // Create depth/stencil attachment image
    vk_image depthImg;
    vkr = vk_create_image(width, height, VK_FORMAT_D32_SFLOAT_S8_UINT,
                          VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
                          &depthImg);
    if (vkr != VK_SUCCESS)
    {
        std::cout << "Warning: Could not create depth image.\n";
        return;
    }

    vkr = vk_track_image(&depthImg, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    if (vkr != VK_SUCCESS)
    {
        std::cout << "Warning: Could not allocate memory for depth image.\n";
        return;
    }

    vkr = vk_create_image_view(&depthImg);
    if (vkr != VK_SUCCESS)
    {
        std::cout << "Warning: Could not create depth image view.\n";
        return;
    }

    // Create render pass with color and depth/stencil
    VkRenderPass renderPass = VK_NULL_HANDLE;
    vkr = vk_create_render_pass_with_depth(&renderPass);
    if (vkr != VK_SUCCESS)
    {
    ...
    vkr = vk_create_framebuffer_with_depth(renderPass, width, height,
                                           colorImg.view, depthImg.view,
                                           &framebuffer);
    if (vkr != VK_SUCCESS)
    {
    ...
    rt.colorImage = colorImg.handle;
    rt.colorView = colorImg.view;
    rt.depthStencil = depthImg;
    m_renderTargets.push_back(rt);

    std::cout << "Depth/stencil render target created successfully.\n";
}
