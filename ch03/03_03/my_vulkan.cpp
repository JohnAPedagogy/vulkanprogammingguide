...
VkResult vk_reset_command_buffer(VkCommandBuffer commandBuffer)
{
    VkResult vkr = VK_INCOMPLETE;

    if (commandBuffer == VK_NULL_HANDLE)
    {
        std::cout << "Warning: No command buffer, cannot reset.\n";
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    ...
    if (vkr != VK_SUCCESS)
    {
        std::cout << "vkResetCommandBuffer failed with result=" << vkr << "\n";
        vkr = VK_ERROR_INITIALIZATION_FAILED;
    }
    return vkr;
}
...
void my_record_copy_recycled(void)
{
    if (m_buffers.size() < 2)
    {
    ...
    // Record, submit, wait twice to demonstrate recycling
    for (int iteration = 0; iteration < 2; ++iteration)
    {
        VkResult vkr = vk_reset_command_buffer(m_commandBuffer);
        if (vkr != VK_SUCCESS)
        {
            std::cout << "Warning: Failed to reset command buffer iteration " << iteration << "\n";
            return;
        }

        vkr = vk_record_copy(m_commandBuffer, source, destination, byteCount);
        if (vkr != VK_SUCCESS)
        {
            std::cout << "Warning: Failed to record copy iteration " << iteration << "\n";
            return;
        }

        vkr = vk_submit_and_wait(m_graphicsQueue, m_commandBuffer, m_fence);
        if (vkr != VK_SUCCESS)
        {
            std::cout << "Warning: Failed to submit and wait iteration " << iteration << "\n";
            return;
        }
    }

    std::cout << "recorded and recycled: yes\n";
}
